...
from moulinette import Utilisateur
...
def test_waterfall_infinite():
    ...
    moulinette = WaterfallMoulinetteInfinite(test_capacity=2, test_time=2, result_time=1)
...
def test_waterfall_finite():
    ...
    moulinette = WaterfallMoulinetteFinite(test_capacity=1, ks=3, kf=2, test_time=2, result_time=1)
    user_list = create_user_list(["Alice", "Bob", "Charlie", "Diana", "Eve"])
    for user in user_list:
        moulinette.add_user(user)
    moulinette.start_simulation(until=20)
...
def test_waterfall_finite_backup():
    ...
    moulinette = WaterfallMoulinetteFiniteBackup(test_capacity=1, ks=3, kf=2, test_time=2, result_time=1, backup_capacity=5)
    user_list = create_user_list(["Alice", "Bob", "Charlie", "Diana", "Eve"])
    for user in user_list:
        moulinette.add_user(user)
    moulinette.start_simulation(until=20)
...
def test_channels_and_dams():
    ...
    """
    print("\n=== Channels and Dams ===\n")
    moulinette = ChannelsAndDams(capacity=2, process_time=2, tb=5)
    user_list = create_user_list(["Alice", "Bob", "Charlie", "Diana", "Eve"])
    for user in user_list:
        moulinette.add_user(user)
...
def test_large_population_waterfall_infinite():
    ...
    moulinette = WaterfallMoulinetteInfinite(test_capacity=5, test_time=1, result_time=1)
...
def test_extreme_load_waterfall_finite():
    ...
    moulinette = WaterfallMoulinetteFinite(test_capacity=2, ks=10, kf=5, test_time=2, result_time=2)
...
def test_variable_backup_capacity():
    ...
    for backup_capacity in capacities:
        ...
        moulinette = WaterfallMoulinetteFiniteBackup(test_capacity=2, ks=5, kf=5, test_time=2, result_time=2, backup_capacity=backup_capacity)
...
def test_different_blocking_times():
    ...
    for tb in blocking_times:
        print(f"\n--- Blocking Time: {tb} ---\n")
        moulinette = ChannelsAndDams(capacity=3, process_time=2, tb=tb)
        user_list = create_user_list([f"User{i}" for i in range(15)], promo_ratio=0.7)  # Plus d'utilisateurs ING
        for user in user_list:
            moulinette.add_user(user)
        moulinette.env.process(moulinette.regulate_ing())
        moulinette.start_simulation(until=50)
...
def test_mixed_load_channels_and_dams():
    ...
    moulinette = ChannelsAndDams(capacity=4, process_time=1, tb=5)
    user_list = create_user_list([f"User{i}" for i in range(40)], promo_ratio=0.6)  # Mélange de 60% ING et 40% PREPA
    for user in user_list:
        moulinette.add_user(user)
    moulinette.env.process(moulinette.regulate_ing())
    moulinette.start_simulation(until=100)
